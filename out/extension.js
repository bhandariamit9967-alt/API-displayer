...
function activate(context) {
    ...
    const getGroqClient = () => {
        ...
        const apiKey = config.get("groqApiKey") ||
            ...
            "";
        if (!apiKey) {
            throw new Error("No Groq API key found. Set it in Settings → Debate Partner → Groq Api Key, " +
                "or export the GROQ_API_KEY environment variable.");
        }
        return new groq_sdk_1.default({ apiKey });
    };
    let panel;
    const openCommand = vscode.commands.registerCommand("debatePartner.open", () => {
        if (panel) {
            panel.reveal(vscode.ViewColumn.Beside);
            return;
        }
        panel = vscode.window.createWebviewPanel("debatePartner", "Debate Partner", vscode.ViewColumn.Beside, {
            enableScripts: true,
            ...
            switch (message.type) {
                case "startDebate": {
                    ...
                    try {
                        ...
                        const opening = await generateOpeningStatement(client, topic, userPosition, difficulty);
                        panel?.webview.postMessage({
                            type: "aiOpening",
                            sessionId,
                            text: opening,
                ...
                case "submitArgument": {
                    const { sessionId, roundNumber, userArgument, topic, userPosition, difficulty } = message;
                    try {
                        const client = getGroqClient();
                        const [score, rebuttal] = await Promise.all([
                            scoreArgument(client, userArgument, topic, difficulty),
                            generateRebuttal(client, userArgument, topic, userPosition, difficulty, roundNumber),
                        ]);
                        const round = {
                            roundNumber,
                            userArgument,
                ...
                case "endDebate": {
                    ...
                    try {
                        const client = getGroqClient();
                        const verdict = await generateVerdict(client, topic, userPosition, difficulty, rounds);
                        const hist = loadHistory(historyFilePath);
                        const session = hist.sessions.find((s) => s.id === sessionId);
                        if (session) {
...
const DIFFICULTY_INSTRUCTIONS = {
    ...
    medium: "Make well-rounded arguments with clear reasoning. Introduce some nuance and a couple of real-world examples.",
    ...
    expert: "Argue like a world-class debate champion. Use rigorous logic, cite studies or historical precedent, expose every logical fallacy, and deliver the most devastating possible rebuttal.",
};
async function generateOpeningStatement(client, topic, userPosition, difficulty) {
...
async function generateRebuttal(client, userArgument, topic, userPosition, difficulty, roundNumber) {
    const aiPosition = userPosition === "for" ? "against" : "for";
...
async function scoreArgument(client, userArgument, topic, difficulty) {
...
async function generateVerdict(client, topic, userPosition, difficulty, rounds) {
    const roundSummary = rounds
        .map((r) => `Round ${r.roundNumber}: User scored ${r.score.overall}/10 — "${r.score.feedback}"`)
        .join("\n");
    const avgScore = rounds.reduce((sum, r) => sum + r.score.overall, 0) / (rounds.length || 1);
